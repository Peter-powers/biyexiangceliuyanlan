...
### API1.使用水平
api类别|功能|输入|输出|重要（优先）等级
:--|:--|:--|:--|:--
人脸检测|生成人员id|用户上传的合照|合照图片中每个人像的id|重要
人脸对比|头像锁定|用户人脸的照片|用户上传的图片对应的人像id|次重要
声纹识别|声纹锁定|用户说话的音频|人像对应的id|次重要
语音合成|收集语音、留言栏|用户输入文字的文本|用户过去的声音来念文字的音频|最重要
